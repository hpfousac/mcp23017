...
HAL_StatusTypeDef mcp23017_write(MCP23017_HandleTypeDef *hdev, uint16_t reg, uint8_t *data)
{
	...
	// while (HAL_OK != HAL_I2C_Mem_Write_IT(hdev->hi2c, hdev->addr, reg, 1, data, 1));
	// return HAL_OK;
}

HAL_StatusTypeDef mcp23017_write_async(MCP23017_HandleTypeDef *hdev, uint16_t reg, uint8_t *data)
{
	while (HAL_OK != HAL_I2C_Mem_Write_IT(hdev->hi2c, hdev->addr, reg, 1, data, 1));
...
HAL_StatusTypeDef mcp23017_write_gpio_async(MCP23017_HandleTypeDef *hdev, uint8_t port)
{
	uint8_t data[1] = {hdev->gpio[port]};
	return mcp23017_write_async(hdev, MCP23017_REGISTER_GPIOA|port, data);
}
